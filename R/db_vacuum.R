...
#' 
#' The PostgreSQL statement uses \code{VACUUM (VERBOSE)} and the MySQL statement
#' uses \code{OPTIMIZE}. 
#' 
...
db_vacuum <- function (con, table) {
  ...
  if (grepl("postgresql", class(con), ignore.case = TRUE))
    quiet(
      db_send(con, stringr::str_c("VACUUM (VERBOSE) ", table))
    )
  ...
  if (grepl("mysql", class(con), ignore.case = TRUE)) {
    ...
    quiet(
      db_send(con, stringr::str_c("OPTIMIZE TABLE ", table))
    )
